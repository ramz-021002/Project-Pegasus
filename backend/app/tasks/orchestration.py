...
# Create Celery app
celery_app = Celery(
    "pegasus", broker=settings.celery_broker_url, backend=settings.celery_result_backend
)
...
def analyze_sample(self, sample_id: str):
    ...
    try:
        ...
        DatabaseOperations.update_sample_status(
            db, uuid.UUID(sample_id), SampleStatus.ANALYZING
        )
        ...
        try:
            DatabaseOperations.update_sample_status(
                db, uuid.UUID(sample_id), SampleStatus.FAILED
            )
        except:
            pass
        return {"error": str(e)}
...
def run_static_analysis(self, sample_id: str, quarantine_path: str):
    ...
    try:
        ...
        success, results, container_id = docker_manager.run_static_analysis(
            temp_path, timeout=settings.analysis_timeout
        )
...
def run_dynamic_analysis(self, static_results, sample_id: str, quarantine_path: str):
    ...
    try:
        ...
        gateway_success, gateway_container_id = docker_manager.run_network_gateway(
            network_id, duration=settings.dynamic_execution_timeout + 30
        )
        ...
        DatabaseOperations.update_sample_status(
            db, uuid.UUID(sample_id), SampleStatus.COMPLETED
        )
...
def cleanup_orphaned_resources():
    ...
    logger.info(
        f"Cleanup complete: {containers_cleaned} containers, {networks_cleaned} networks"
    )
